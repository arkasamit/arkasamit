
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=7F77DD&height=200&section=header&text=Arka%20Sarker%20Amit&fontSize=40&fontColor=ffffff&fontAlignY=35&desc=Senior%20IT%20Officer%20%7C%20MERN%20%26%20Next.js%20Developer&descAlignY=55&descSize=16" />
</p>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com/?lines=Full+Stack+Developer;MERN+Stack+Enthusiast;Next.js+Learner;Always+Learning+New+Things&center=true&width=500&height=45&color=7F77DD" />
</p>



## 💻 Tech Stack

<p align="left">
  <img src="https://skillicons.dev/icons?i=html,css,js,php,react,nextjs,nodejs,express,mongodb,mysql,redux,tailwind,bootstrap,docker,figma,vercel,cloudflare" />
</p>

---

## 🌐 Socials

<p align="left">
  <a href="https://discord.gg/arkaamit"><img src="https://img.shields.io/badge/Discord-%237289DA.svg?logo=discord&logoColor=white" /></a>
  <a href="https://facebook.com/arkasamit"><img src="https://img.shields.io/badge/Facebook-%231877F2.svg?logo=Facebook&logoColor=white" /></a>
  <a href="https://linkedin.com/in/arkasamit"><img src="https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white" /></a>
  <a href="mailto:arkaamitdev@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white" /></a>
  <a href="https://www.arkasamit.dev"><img src="https://img.shields.io/badge/Portfolio-%23000000.svg?logo=firefox&logoColor=white" /></a>
</p>


## 🚀 Currently Building

<img src="https://capsule-render.vercel.app/api?type=rect&color=EEEDFE&height=80&text=Full-Stack%20Web%20App%20with%20Next.js%20%2B%20MongoDB&fontSize=18&fontColor=3C3489&desc=Scalable%20dashboard%20with%20auth%2C%20REST%20API%2C%20and%20responsive%20UI&descSize=13&fontAlignY=38&descAlignY=65" />

<p>
  <img src="https://img.shields.io/badge/Next.js-black?style=flat-square&logo=next.js" />
  <img src="https://img.shields.io/badge/MongoDB-4ea94b?style=flat-square&logo=mongodb&logoColor=white" />
  <img src="https://img.shields.io/badge/TailwindCSS-38B2AC?style=flat-square&logo=tailwind-css&logoColor=white" />
  <img src="https://img.shields.io/badge/Node.js-6DA55F?style=flat-square&logo=node.js&logoColor=white" />
</p>

---

## 🧠 Skills & Comfort Level

| Tech | Level |
|------|-------|
| Tailwind CSS | ████████░░ 80% |
| React / Next.js | ████████░░ 72% |
| Node.js / Express | ███████░░░ 65% |
| MongoDB | ██████░░░░ 60% |
| Docker | ████░░░░░░ 35% |

---

## 💡 I Can Build

| | What | Description | Tech Stack |
|-|------|-------------|------------|
| 🛒 | E-Commerce App | Full-stack store with cart, auth & payment integration | MERN, Redux, Stripe |
| 📊 | Admin Dashboard | Data reporting & analytics with role-based access | Next.js, MySQL, Chart.js |
| 💬 | Real-Time Chat App | Live messaging with rooms & JWT auth | Node.js, React, Socket.io |
| 🔐 | Auth System | Secure login with JWT, OAuth & refresh tokens | Express, MongoDB, NextAuth |
| 📝 | Blog / CMS | Content management with rich text editor & SEO | Next.js, MongoDB, Tailwind |
| 🗂️ | Task Manager | Kanban-style project tracker with drag & drop | React, Redux, Node.js |

---

## 🟢 Open To

<p>
  <img src="https://img.shields.io/badge/Freelance-Open-brightgreen?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Remote%20Roles-Open-185FA5?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Open%20Source-Collaborating-3B6D11?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Mentorship-Welcome-BA7517?style=for-the-badge" />
</p>

---

### ✍️ Random Dev Quote
![](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=radical)

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=7F77DD&height=100&section=footer" />
</p>

[![](https://visitcount.itsvg.in/api?id=arkasamit&icon=0&color=0)](https://visitcount.itsvg.in)
